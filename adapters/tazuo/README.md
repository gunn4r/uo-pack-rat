...
- **A container showing a name plate cannot be closed by a script.** TazUO's `GetContainerGump()` asks `UIManager.GetGump(serial)`, which walks the client's gump list from its `Last` node and returns the first gump of any kind with that serial, then checks it is a container window. `UIManager.Add()` puts a new window in front (`AddFirst`), but an item's name plate (All Names / object handles, or permanently toggled name plates) is added with `front = false` (`AddLast`), behind every window, so while the item has a plate the lookup finds the plate and answers nothing, whenever it is asked. Ground containers with a plate therefore stay open (bags inside them have no plate and close), and the scan says how many in the game's system messages. Turn name plates off before a scan if that bothers you. Reported upstream as [PlayTazUO/TazUO#1087](https://github.com/PlayTazUO/TazUO/issues/1087), fix proposed in [PR #1088](https://github.com/PlayTazUO/TazUO/pull/1088) (look the window up by type).
