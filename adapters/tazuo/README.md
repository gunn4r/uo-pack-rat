...
- The scanner and the refresh close every container window they opened themselves once the scan file is written (or after a Stop or an error), innermost first; a window that was already open before the run (the item's `Opened` flag, read just before the script opens it) is left open. Each window is taken hold of right after the script's own `UseObject`, by polling the item's `GetContainerGump()` during the open pause, and closed at the end with that window's `Dispose()`; one not taken then is looked up once more at the end. Both calls are looked up with `getattr`, so a client build without either leaves the windows open instead of failing. `API.CloseGump(serial)` is not used: it finds gumps by their server gump id, which a container window does not have, so it cannot close one. The bridge still leaves open whatever it opened.
- **A container showing a name plate cannot be closed by a script.** TazUO's `GetContainerGump()` asks `UIManager.GetGump(serial)`, which walks the client's gump list from the back and returns the first gump of any kind with that serial, then checks it is a container window. An item's name plate (All Names / object handles, or permanently toggled name plates) has the item's serial and is always added at the back of that list, so while the plate exists the lookup finds the plate and answers nothing, however soon after opening it asks. A window taken before its plate appeared still closes; with name plates showing when the scan starts, ground containers (bags inside them have no plate) stay open, and the scan says how many in the game's system messages. Turn name plates off before a scan if that bothers you. The fix belongs in TazUO: look the window up by type (`UIManager.GetGump<ContainerGump>(serial)`, `GridContainer`, `GridLootGump`).
