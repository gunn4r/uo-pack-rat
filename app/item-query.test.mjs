...
import { EXTRA_COLS, colVal, rarityRank, parseItemQuery, applyItemQuery, facetsOf } from "./item-query.mjs";
import { KINDS } from "./vault-lib.mjs";
...
const names = (r) => r.map((it) => it.name);
...
test("[fast] applyItemQuery: text search hits itemSearchBlob (name)", () => {
  ...
  const { rows } = applyItemQuery(ITEMS, q, ctx);
  assert.deepEqual(names(rows), ["Vile Ring"]);
});

test("[fast] applyItemQuery: kind filter", () => {
  const { rows } = applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("kind=bandage")), ctx);
  assert.deepEqual(names(rows), ["Bandage"]);
});

test("[fast] applyItemQuery: slot filter, including \"?\" for unknown slot", () => {
  assert.deepEqual(names(applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("slot=ring")), ctx).rows), ["Vile Ring"]);
  const unknownSlot = applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("slot=%3F")), ctx).rows;
...
test("[fast] applyItemQuery: location filter", () => {
  const { rows } = applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("loc=" + encodeURIComponent("Dorran's bank"))), ctx);
  assert.deepEqual(names(rows).sort(), ["Cloth Robe", "Leather Gloves"]);
});

test("[fast] applyItemQuery: rarity filter", () => {
  const { rows } = applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("rarity=" + encodeURIComponent("Lesser Artifact"))), ctx);
  assert.deepEqual(names(rows).sort(), ["Silver Katana", "Vile Ring"]);
});

test("[fast] applyItemQuery: seenDays cuts off anything older", () => {
  const { rows } = applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("seenDays=5")), ctx);
...
test("[fast] applyItemQuery: slayer filter, including \"*\" for any slayer", () => {
  assert.deepEqual(names(applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("slayer=Orc")), ctx).rows).sort(), ["Orc Slayer Cutlass", "Vile Ring"]);
  assert.deepEqual(names(applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("slayer=%2A")), ctx).rows).sort(), ["Composite Bow", "Orc Slayer Cutlass", "Silver Katana", "Vile Ring"]);
});

test("[fast] applyItemQuery: nogarg excludes gargoyle-only gear", () => {
  const { rows } = applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("nogarg=1")), ctx);
...
test("[fast] applyItemQuery: med excludes non-meditation-safe gear", () => {
  const { rows } = applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("med=1")), ctx);
...
test("[fast] applyItemQuery: hideTags drops a tagged item even if it would otherwise match", () => {
  const { rows } = applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("hide=cursed")), ctx);
  assert.ok(!names(rows).includes("Vile Ring"));
});

test("[fast] applyItemQuery: prop min filters on colVal, including EXTRA_COLS keys", () => {
  const { rows } = applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("prop=hci:15")), ctx);
  ...
  assert.equal(colVal({ strReq: 40, props: {} }, "strReq"), 40);
  assert.equal(colVal({ weight: 3, props: {} }, "weight"), 3);
...
test("[fast] applyItemQuery: sort by a prop column — dir=1 is highest-first, dir=-1 is lowest-first", () => {
  ...
  const desc = applyItemQuery(withHci, parseItemQuery(new URLSearchParams("sort=hci")), ctx).rows;
  assert.deepEqual(names(desc), ["Orc Slayer Cutlass", "Composite Bow", "Silver Katana", "Vile Ring"]);
  const asc = applyItemQuery(withHci, parseItemQuery(new URLSearchParams("sort=hci&dir=-1")), ctx).rows;
...
test("[fast] applyItemQuery: sort by rarity uses the ladder order, not alphabetical", () => {
  ...
  const { rows } = applyItemQuery(withRarity, parseItemQuery(new URLSearchParams("sort=rarity&dir=-1")), ctx);
...
test("[fast] applyItemQuery: paging — offset/limit slice rows, total and pieces cover every match", () => {
  ...
  const all = applyItemQuery(ITEMS, parseItemQuery(new URLSearchParams("kind=gear")), ctx);
  const page = applyItemQuery(ITEMS, q, ctx);
...
test("[fast] applyItemQuery: group mode shapes rows as JSON-safe groups and sorts by amount/kind/name", () => {
  ...
  const byAmount = applyItemQuery(items, parseItemQuery(new URLSearchParams("group=1&sort=amount")), ctx);
  ...
  const byName = applyItemQuery(items, parseItemQuery(new URLSearchParams("group=1&sort=name")), ctx);
