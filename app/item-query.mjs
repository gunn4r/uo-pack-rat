// item-query.mjs — pure item-list filtering/sorting/paging/faceting, shared by the browser (ui/dom.mjs
...
// node: imports — this file is served to the browser byte-for-byte, the same way vault-lib.mjs is.
//
...
import { itemSearchBlob, groupByName, KINDS, SLOT_LABELS, propertyKeys, gearSkills } from "./vault-lib.mjs";

// Columns computed from an item but not stored under item.props — moved verbatim from ui/dom.mjs (Task 4).
export const EXTRA_COLS = { strReq: ["STR req", "Strength Requirement"], weight: ["Wt", "Weight (stones)"] };
export const colVal = (it, c) => (c === "strReq" ? it.strReq || 0 : c === "weight" ? it.weight || 0 : it.props[c] || 0);
...
export function rarityRank(ladder, name) {
  const i = (ladder || []).findIndex((r) => r.name.toLowerCase() === String(name || "").toLowerCase());
  return i >= 0 ? i + 1 : 0;
}

const CLAMP_LIMIT = (n) => Math.max(1, Math.min(500, n));
...
export function parseItemQuery(searchParams) {
  ...
  const splitAll = (name) => sp.getAll(name).flatMap((v) => String(v).split(",")).map((s) => s.trim()).filter(Boolean);
...
function sortValue(it, key, ladder) {
  ...
  if (key === "slot") return SLOT_LABELS[it.slot] || it.slot || "?";
  if (key === "location") return it.location?.text || "";
  return colVal(it, key);
}

const groupJson = (g) => ({ name: g.name, kind: g.kind, slot: g.slot, amount: g.amount, stacks: g.stacks, locations: [...g.locations.entries()] });
...
export function applyItemQuery(items, query, { rarity = [], now = Date.now() } = {}) {
...
// the one place left for the page to get it, instead of calling vault-lib.mjs's gearSkills()
...
export function facetsOf(items, { rarity = [] } = {}) {
  const slots = [...new Set(items.map((i) => i.slot).filter(Boolean))].sort();
  const locations = [...new Set(items.map((i) => i.location?.text).filter(Boolean))].sort();
  const rarities = [...new Set(items.map((i) => i.rarity).filter(Boolean))].sort((a, b) => rarityRank(rarity, a) - rarityRank(rarity, b));
