...
- `run-bench.mjs` — the sweep. Every optimizer call runs in `app/optimize-worker.mts` (the server's worker) under a hard wall cap, because the core's `timeBudgetMs` only bounds the exact phase.
