...
const fixture = upgradeScan(fixtureRaw, { shard: "uoalive" });
...
const defaultProfiles = JSON.parse(readFileSync(join(HERE, "data", "profiles.default.json"), "utf8"));
...
function cell(profileName, { soft = [], overrides = {} } = {}) {
  ...
  return { pools: fixturePools, current: fixtureCurrent, profile };
}

const sig = (assignment) => DEFAULT_SLOTS.map((s) => (assignment && assignment[s] ? assignment[s].serial : null)).join(",");
...
async function runBoth(t, { pools, current, profile }, opts) {
...
const BASE_OPTS = { exact: true, timeBudgetMs: 20000, restarts: 50, seed: 2026 };
...
test("[fast] k-best matches the core's alternatives score for score; no alternative equals the best's serial set", async () => {
  ...
  const opts = { ...BASE_OPTS, alternatives: { count: 3, tolerance: 1e9 } };
  const r = await solveExact({ core, pools, current, profile, opts, onProgress: () => {} });
  const ref = core.optimizeSuit(pools, current, profile, opts);
  assert.equal(r.solver, "highs");
...
test("[fast] jointly unreachable hard floors fall back honestly", async () => {
  ...
  const opts = { exact: true, timeBudgetMs: 5000, restarts: 20, seed: 1, slots, optionalSlots };
  const r = await solveExact({ core, pools, current, profile, opts, onProgress: () => {} });
...
test("[fast] a negative total on a soft-floored dimension gets zero credit, like the core", async (t) => {
  ...
  const profile = { weights: { manaRegen: 1, hci: 1 }, caps: {}, floors: { manaRegen: 2 }, hardFloors: [], floorBonus: 1000 };
  const opts = { exact: true, timeBudgetMs: 5000, restarts: 20, seed: 1, slots, optionalSlots };
...
test("[fast] HiGHS unavailable → the heuristic result, flagged", async () => {
  const name = templateNames[0];
  const { pools, current, profile } = cell(name);
  const opts = { ...BASE_OPTS, timeBudgetMs: 5000 };
...
test("[fast] a plain solver timeout with no incumbent is reported honestly, not as a floors conflict", async () => {
  ...
  const { pools, current, profile } = cell(name);
  const opts = { ...BASE_OPTS, timeBudgetMs: 5000 };
  const stubSolveModel = () => ({ status: "timeLimit", statusText: "timeLimit", objective: null, primal: null, dual: null, gapAbs: null, nodes: 3, colValue: null, ms: 1 });
  ...
  const heur = core.optimizeSuit(pools, current, profile, { ...opts, exact: false, restarts: opts.restarts });
...
test("[fast] a plain solver timeout still reports a bound when the solver found one", async () => {
  ...
  const { pools, current, profile } = cell(name);
  const opts = { ...BASE_OPTS, timeBudgetMs: 5000 };
  const stubSolveModel = () => ({ status: "timeLimit", statusText: "timeLimit", objective: null, primal: null, dual: 123.5, gapAbs: null, nodes: 7, colValue: null, ms: 1 });
  const r = await solveExact({ core, pools, current, profile, opts, onProgress: () => {}, solveModel: stubSolveModel });
  assert.equal(r.solver, "highs");
  assert.equal(r.floorsConflict, false);
...
test("[fast] the exact result never scores below the heuristic, even on a poor timeLimit incumbent", async () => {
  ...
  const opts = { ...BASE_OPTS, timeBudgetMs: 5000 };
  const heur = core.optimizeSuit(pools, current, profile, { ...opts, exact: false, restarts: opts.restarts });
  const stubSolveModel = (handle) => {
    const colValue = new Float64Array(handle.built.model.numCols);   // every column 0: nothing picked anywhere
    return { status: "timeLimit", statusText: "timeLimit", objective: -1e9, primal: -1e9, dual: null, gapAbs: null, nodes: 5, colValue, ms: 1 };
  };
  const r = await solveExact({ core, pools, current, profile, opts, onProgress: () => {}, solveModel: stubSolveModel });
  assert.equal(r.solver, "highs");
...
test("[slow] a 3,000-item generated cell: HiGHS proves and the core agrees where it proves",
  { skip: process.env.TEST_SKIP_SLOW === "1" }, async () => {
    ...
    const genScan = upgradeScan(genRaw, { shard: "uoalive" });
    ...
    const opts = { exact: true, timeBudgetMs: 60000, restarts: 50, seed: 2026 };
    const r = await solveExact({ core, pools, current, profile, opts, onProgress: () => {} });
    assert.equal(r.solver, "highs");
