// ui/view-state.mts — the page's pure state rules: what a refresh keeps and what "Clear all" resets. No DOM and no store.mts import, so app/ui-state.test.mts imports it under
...
// A filter checklist's options after a refresh rebuilt them from the new facets, keeping a value the
// query still applies. When that value has left the facets (the last matching item was forgotten),
// it stays as an option so the list keeps showing the filter the table is still using, rather than
// dropping it while the rows stay filtered.
