// ui/dialog.mts — small reusable in-page dialogs, built on the native <dialog> element. Exists
// because Electron does not implement window.prompt() (it silently returns null/undefined with no
// error) — ui/builder.mts's saveTemplateAs used to call prompt() directly and simply did nothing in
// the desktop app, while working fine in a plain browser. confirm() DOES work in Electron and is left
// alone everywhere it's already used (the overwrite/delete confirmations in ui/builder.mts).
...
export function promptText({ title = "", value = "", okLabel = "Save", placeholder = "" }: PromptTextOptions = {}): Promise<string | null> {
  return new Promise((resolve) => {
    ...
    const submit = (): void => done(normalizePromptValue(input.value));
    ...
    function done(v: string | null): void { dialog.close(); dialog.remove(); resolve(v); }
