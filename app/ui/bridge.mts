...
// Why a bridge action can't run right now, in words for a disabled control's tooltip, or null when it can:
// the client has no such action, or the bridge isn't answering.
export const BRIDGE_OFFLINE_REASON = "Bridge offline. Press Play on packrat-bridge.py in game.";
export function bridgeGate(action: "highlight" | "grab" | "goto"): string | null {
  if (!allowedBridgeActions().includes(action)) return `${currentAdapter()?.name || "This client"} can't ${ACTION_LABELS[action]!.toLowerCase()} from Pack Rat.`;
  return bridge.online ? null : BRIDGE_OFFLINE_REASON;
}
