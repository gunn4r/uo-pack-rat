...
- **App ⇄ game client, the scan blacklist.** `<data>/scan-blacklist.json` lists the containers scans skip; the app (`/api/blacklist`) and TazUO's `packrat-blacklist.py` write it, and every scanner reads it when it starts.
...
### 9. The scan blacklist → the scanners
...
**Trusted:** nothing it reads. `packrat-panel.py` starts and stops only the four Pack Rat scripts it names itself, in its own folder; nothing read from a file becomes a script name or a path. The one file it acts on, `<data>/tazuo-panel.json` (the hotkey, and the open-at-login choice its toggle writes there as pending), is size-capped on both sides, and the app honours a pending flag only beside a real true/false choice; the hotkey must be a known modifier set plus A–Z, 0–9 or F1–F12, else the default is used. It takes no action in the world: every world action still comes from a click, on its buttons or in the app. Its heartbeat `panel.json` joins the bridge's `status.json` in the installer's running-script guard, so a forged heartbeat can only make an install refuse for 30 seconds.

**The one TazUO file the app edits.** "Open the panel at login" adds or removes `packrat-panel.py` in `GlobalAutoStartScripts` in `<TazUO>/Data/lscript.json` (`<TazUO>` is the folder holding the configured LegionScripts folder). It is only made for a choice the player just made in Settings or the wizard, never replayed on its own, so unticking Autostart in game sticks. It only happens while no TazUO process is running (`pgrep -x 'TazUO(\.exe)?'`, or `tasklist` for `TazUO.exe` on Windows; an unclear answer counts as running), because a running client saves its own copy at logout; a choice made while it runs is kept as pending and retried at app start, when Settings opens, and on the next save or install. The file is looked for only beside the nearest folder named `LegionScripts` above the scripts folder, and never created anywhere else. It merges: every other key and entry is kept, an existing BOM is kept, a file that is not the expected shape is left alone, and the replaced file is copied to `lscript.json.bak` first. The only name it ever adds is the fixed `packrat-panel.py`.
