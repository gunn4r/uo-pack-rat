...
Every response that goes through the server's one `send` helper carries `x-content-type-options: nosniff`, `cache-control: no-store` and `x-frame-options: DENY`; the HTML response also carries the CSP, which includes `frame-ancestors 'none'`. (The two SSE routes write their own headers and so carry neither — a stream of `text/event-stream` is not a document anything can frame.) No CORS header is ever sent, so a cross-origin read is refused by the browser itself.
...
The utility-process split exists so that the code which parses untrusted scan files and runs third-party adapter watchers cannot reach `BrowserWindow`, `shell`, or the OS. Two operations cross back, and `electron/host-args.mts` checks both before either reaches an OS call. `openPath` takes a **discriminator** — `"data"` or `"logs"` — which main maps to the two directories it computed itself; a message can never name a third path for the OS to launch. (A path byte-identical to one of those two is still accepted, so an older server that resolved the path itself keeps working.) `pickFolder`'s `title` is coerced to a short, control-character-free string before it can become the caption on an app-modal native dialog.
...
**Log files are not rotated.** `safeAppendLog` appends and never trims, and the shell's own logger does the same; `server.log` and `shell.log` both grow for as long as the app is used. Neither holds a credential — the token is never printed — and deleting either is safe at any moment. The cost of the miss is disk, not disclosure. Related and recorded with it: `server.log` is written owner-only, while `shell.log` and the `logs/` directory are created by the shell before the server runs and take the process default, so on a shared machine `shell.log` is the one file here another account can read. It carries the server's stdout, not request contents.
