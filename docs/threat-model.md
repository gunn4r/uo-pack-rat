...
- **App ⇄ game client, the scan blacklist.** `<data>/scan-blacklist.json` lists the containers scans skip. The app writes it (`POST /api/blacklist`, `DELETE /api/blacklist/<serial>`), and so does TazUO's `packrat-blacklist.py`; every scanner reads it when it starts, and the server on every fold.
...
- **At the door.** `ingestFile` `lstat`s before it reads: anything that is not a regular file (a symlink, a directory, a device) is refused unread, and so is anything over 32 MB. A rejection reason — from the inbox or from a paste — no longer quotes the file's own bytes back into a message the page displays.
...
### 9. The scan blacklist → the scanners and the fold

**Trusted:** nothing in the file. It is written by the app and by an in-game script, and it is an ordinary file anything on the machine could write.

The file can only ever make a scan skip containers: a listed serial is never double-clicked, recorded or descended into, and nothing is opened, moved or deleted because of the list. Both readers apply the same shape — a JSON list of at most 1000 `{serial, name, addedAt, where?}` entries in at most 256 KB, serials positive 32-bit integers, strings bounded — and a file that fails any of it is ignored whole: the scanners scan everything (`read_blacklist` answers None, never raises), the server folds with no list and logs why (`readBlacklist`), `GET /api/blacklist` reports the problem so Settings can say it, and a later add from the app moves the bad file aside rather than writing over it. `packrat-blacklist.py` refuses to touch an unreadable list, and never adds the player's own backpack or bank. The fold's use of the list is limited to scans dated after an entry's `addedAt`, and never reaches the app's own tombstones, so a hostile entry can hide a container's newer contents from the page but cannot remove what an earlier scan recorded or stop a Forget. The app and the script both read, append and atomically replace the file; two writes at the same instant can lose one entry, which re-adding fixes.

**Load-bearing:** **skip is the only verb the file has.** Neither side acts on a listed serial beyond not reading it, so the worst a forged list does is leave containers out of Pack Rat.
