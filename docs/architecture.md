...
`POST /api/setup/install {adapter, scriptsDir}` (backing the wizard's Install step and the Settings tab's Reinstall button, folder-transport adapters only — see below) copies an adapter's scripts into the player's chosen scripts folder (TazUO's `LegionScripts/`, Razor Enhanced's `Scripts/`) and writes `packrat-paths.json` there so the scripts know where this app's data directory is. Before touching anything on disk it checks the adapter's own `capabilities.json` `transport` field and refuses with `code: "noInstall"` for a `"paste"`-transport adapter (docs/adapter-guide.md) — there is no scripts folder to write to at all, so the wizard sends that player to the Import tab instead and never reaches this route. For a folder-transport adapter it then checks the bridge's own `status.json` for a script that looks like it's still running in the client (an `alive` timestamp inside the last 30 s, not `stopped: true`) and refuses with a 409 naming the `-stopall` fix if so — overwriting a script file while a Legion script thread is mid-run against it is exactly the "orphaned script thread" trap the TazUO adapter's own README warns about, so the guard runs first, before any file is copied. Each script is copied to `<name>.new` and renamed into place (never written in place), so a reader never sees a half-written script mid-install; `packrat-paths.json` is written last, once every script is in position, so an interrupted install never leaves a paths file pointing a partial script set at data it doesn't yet match. `app/installer.mjs`'s `candidateClientRoots`/`validateScriptsDir` propose and validate each adapter's own folder shape (`NESTED_SCRIPTS_SUFFIX`) rather than one TazUO-shaped guess for every adapter.

## Capability-driven bridge controls

The page never hard-codes which client is running. `GET /api/setup` returns both `settings.client` (`{adapter, scriptsDir} | null`, which adapter Settings has configured) and `adapters` (every `adapters/<id>/`'s own `capabilities.json`, `listAdapters()`'s output, capabilities included) in one response, cached as `state.setup` at page load and refreshed whenever the wizard or the Settings tab changes it. `app/ui/bridge.mjs`'s `currentAdapter()` looks `state.setup.settings.client.adapter` up in `state.setup.adapters` to get that adapter's `capabilities.bridge` — the list of actions (`"highlight" | "grab" | "goto"`) its bridge script actually implements. `actButtons()` and `grabAllRow()` only render the buttons that list contains, and `bridgeNote()`/`bridgeNoteEl()` produce the one-line explanation (no client configured, or which of the three actions this client's adapter doesn't support) that the Inventory tab, the Suit Builder's result panel, and the Settings tab's Client panel each show once rather than leaving a bridge-less player to notice a button is silently missing. Only one client is ever physically connected at a time, so this is deliberately a single global decision — not something recomputed per item from whichever character's scan happened to produce it (an early version did that, which let a bridge-less character's rows still show buttons whenever any other scanned character's adapter had one).

`ADAPTERS_DIR` (`app/config.mjs`'s `paths.adaptersDir`, default `adapters/` at the repo root) is resolved through `resolveConfig()` like every other path — overridable with `--adapters <dir>` or `PACKRAT_ADAPTERS_DIR`, the same pattern `PACKRAT_CORE` uses — so a test can start a real server against a throwaway folder of fixture adapters (one with a full bridge, one with none) instead of the repo's real ones; see `app/server.test.mjs`'s `GET /api/setup: an adapter with no bridge...` test.
