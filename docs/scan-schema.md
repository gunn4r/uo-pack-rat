...
- **Location text tells same-named containers apart.** An item's `location.text` is the container path (`Metal Chest › A Bag`); the page's Location filter, the group view's per-place counts and the Suit Builder's Fetch list all key on it. When containers that sit side by side share a label (two ground roots called "Metal Chest", three bags called "A Bag" in one backpack), each gets a suffix: its position (`Metal Chest (1500, 1600)`) when that alone tells them apart, else its serial (`A Bag (0x4000002b)`). Each folded container carries the result as `label`. A container whose label is unique keeps it plain.
