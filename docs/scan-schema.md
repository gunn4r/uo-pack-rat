...
- **A nested container marked `opened: false` keeps its old contents.** Inside a root that *is* replaced, every item and bag the fold knew within such a container — however deep — is kept, with its old `seenAt`, while everything else in the root is replaced as usual. The container itself is refreshed from the new scan. Once a later scan opens it, its contents are replaced like any other.
