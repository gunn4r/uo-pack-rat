...
- **`opened: false` is invisible to the fold.** A root the adapter saw but could not fully read (too far, locked, a bag inside it that did not open) is listed in `roots[]` with `opened: false` and no items for it. The fold treats this exactly like the scan hadn't mentioned that root at all — it does **not** clear the root's previously-known contents. (Every root a v1 scan lists is upgraded to `opened: true`, so this only matters for scans written by the v2-aware scanner.)
