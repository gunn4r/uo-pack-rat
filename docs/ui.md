...
A Fetch list row shows its place whole, never cut: the path of containers as crumbs that wrap (`builder-model.mts`'s `locationCrumbs`), selectable so any part can be copied, a same-named chest's serial in faint mono, then the container serial with a copy button (components.mts's `copyText`: the Clipboard API, else a selected off-screen textarea and `execCommand("copy")`, since the desktop app's session denies the clipboard permission) and the piece count; the pieces under it are each an item tooltip target (`tipTarget` in `builder-result.mts`, also on the current suit's rows).
