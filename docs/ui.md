...
- **Preferences.** The chosen columns (`cols`), the column set they were saved against (`colsVersion`: a list saved before a column joined the defaults gets it once, then an "off" sticks — `view-state.mts`'s `colsFromPrefs`) and the row density (`density`: `dense` 32 px or `regular` 40 px) are ui-prefs fields.
