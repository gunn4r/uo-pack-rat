...
| `capabilities.json` | This adapter's contract, as data: `{adapter, version, transport, capabilities: {...}}` matching the shape of a scan's own `adapter.capabilities` (see `docs/scan-schema.md`). The scripts' own `CAPABILITIES` dict (whatever they're written in) must match this file exactly — that's what the contract test checks. |
