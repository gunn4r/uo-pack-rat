...
Only those fields are rewritten, and a name also turns up elsewhere (a container called "<name>'s Backpack", an item name, a tooltip line). So the tool searches everything it is about to write for the source character name and account id, in any case, and for each word of a multi-word character name on its own (three letters or more, apart from "the" and "and"), and refuses to write the fixture if either is still there, listing the JSON path of each hit. Rename those in a copy of the scan and run it again.
