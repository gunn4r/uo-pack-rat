...
The server creates its own folders owner-only (`0700`) and writes settings, profiles, saved runs, the bridge queue and `server.log` owner-only (`0600`), so another account on a shared machine cannot read them — and cannot reach your scans either, since the folder holding them is owner-only too. The exception, on the desktop app: the data directory itself and `logs/` are created by the shell before the server starts, with your system's ordinary default permissions, which leaves `logs/shell.log` readable by other accounts on a machine you share.
